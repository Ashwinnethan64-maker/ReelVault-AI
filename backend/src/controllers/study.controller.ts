...
export const getDashboardAnalytics = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const totalFlashcards = await prisma.flashcard.count({ where: { userId } });
    const dueFlashcards = await prisma.flashcard.count({ where: { userId, nextReviewDate: { lte: new Date() } } });
    const quizzesTaken = await prisma.quiz.count({ where: { userId } });
    const totalStudySessions = await prisma.studySession.count({ where: { userId } });

    // Calculate knowledge score (arbitrary metric based on spaced repetition ease factors and quiz scores)
    const cards = await prisma.flashcard.findMany({ where: { userId }, select: { easeFactor: true, repetitions: true } });
...
export const generateFlashcards = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (topic) {
      ...
      reels = await prisma.$queryRawUnsafe<any[]>(`
...
export const saveFlashcards = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user?.id as string;
...
export const generateQuiz = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const context = reels.map((r: any) => `Title: ${r.title}\nSummary: ${r.aiSummary}\nTakeaways: ${r.keyTakeaways ? r.keyTakeaways.join(', ') : ''}`).join('\n\n');
...
export const chatWithTutor = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user?.id as string;
